use advent_of_code_2025::puzzles::day7;
...
fn day7_part1() {
    ...
    let (part1, _) = day7::result(
        &input_path.to_str().unwrap(),
        &output_path.to_str().unwrap(),
    )
    .unwrap();
    assert_eq!(part1, 21, "part 1 mismatch for DAY {DAY}");
}
...
fn day7_part2() {
    let input_path = get_test_input_path(YEAR, DAY, None);
    let output_path = get_test_results_path(YEAR, DAY, None);
    let (_, part2) = day7::result(
        &input_path.to_str().unwrap(),
        &output_path.to_str().unwrap(),
    )
    .unwrap();
    assert_eq!(part2, 40, "part 2 mismatch for DAY {DAY}");
}
