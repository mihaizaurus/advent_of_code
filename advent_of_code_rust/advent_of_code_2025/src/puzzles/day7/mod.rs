...
pub fn result(input_path: &str, output_path: &str) -> io::Result<PuzzleAnswer> {
    let tachyon_manifold_grid = match get_input_as_grid(input_path) {
        Ok(input) => input,
        Err(err) => {
    ...
    let mut tmg_part1 = tachyon_manifold_grid.clone();
    let mut tmg_part2 = tachyon_manifold_grid.clone();

    let part1 = part1::result(&mut tmg_part1);
    let part2 = part2::result(&mut tmg_part2);
