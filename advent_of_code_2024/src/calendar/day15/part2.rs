use advent_of_code_common::{types::{Direction, Position, SimpleGrid, DIRECTIONS}, utils::{get_char_at, set}};
...
fn can_move(warehouse_map: &SimpleGrid, crate_to_check: Crate, direction: Direction) -> bool {
    match direction {
        ...
        Direction(0,1) | Direction(0,-1) => {
            ...
            if can_move_left {
                match get_char_at(crate_to_check.right + direction, warehouse_map).as_str() {
                    "[" => {
                        let next_crate = Crate {
                    ...
                    "]" => {
                        panic!("This is a mistake, wrong situation"); 
                    },
                    "." => true,
                    "#" => false,
