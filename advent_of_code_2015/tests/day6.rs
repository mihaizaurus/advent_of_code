use advent_of_code_2015::puzzles::day6;
...
fn day6_part1() {
    ...
    let (part1, _) = day6::result(
        &input_path.to_str().unwrap(),
        &output_path.to_str().unwrap(),
    )
    .unwrap();
    assert_eq!(part1, 4277556, "part 1 mismatch for DAY {DAY}");
}
...
fn day6_part2() {
    let input_path = get_test_input_path(YEAR, DAY, None);
    let output_path = get_test_results_path(YEAR, DAY, None);
    let (_, part2) = day6::result(
        &input_path.to_str().unwrap(),
        &output_path.to_str().unwrap(),
    )
    .unwrap();
    assert_eq!(part2, 3263827, "part 2 mismatch for DAY {DAY}");
}
