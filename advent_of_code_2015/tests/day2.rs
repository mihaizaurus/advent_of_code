use advent_of_code_2015::puzzles::day2;
...
fn day2_part1() {
    ...
    let (part1, _) = day2::result(
        &input_path.to_str().unwrap(),
        &output_path.to_str().unwrap(),
    )
    .unwrap();
    assert_eq!(part1, 1227775554, "part 1 mismatch for DAY {DAY}");
}
...
fn day2_part2() {
    let input_path = get_test_input_path(YEAR, DAY, None);
    let output_path = get_test_results_path(YEAR, DAY, None);
    let (_, part2) = day2::result(
        &input_path.to_str().unwrap(),
        &output_path.to_str().unwrap(),
    )
    .unwrap();
    assert_eq!(part2, 4174379265, "part 2 mismatch for DAY {DAY}");
}
